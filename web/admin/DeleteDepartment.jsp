...
<html>
  ...
  <body>
   <%
   		...
   		Query q = sess.createQuery("From DepartmentPOJO where id = :i");
   		q.setString("i",id);
   		List<DepartmentPOJO> heli = (List<DepartmentPOJO>)q.list();
   		t.commit();
   		//sess.close();
   		
   		for(DepartmentPOJO e : heli)
   		{
   		...
   		Query q1 = sess1.createQuery("From SubjectPOJO where department = :d");
   		q1.setString("d",department);
   		List<SubjectPOJO> heli1 = (List<SubjectPOJO>)q1.list();
   		t1.commit();
   		//sess1.close();
   		...
   		if(heli1.size()>0)
   		{
   			response.sendRedirect("DeleteMessage.jsp?msg=You can not delete this Department because it is not empty..&status=0");
   		}
   		...
   		Query q2 = sess2.createQuery("Delete from DepartmentPOJO where id = :i");
   		q2.setString("i",id);
   		int result = q2.executeUpdate();
   		t2.commit();
   		sess2.close();
   		
   		response.sendRedirect("DeleteMessage.jsp?msg=Department Successfully Deleted..&status=1");
